...
MT_Init:
	...
	lea	Variables,a5
;;****	move.l	a0,MT_SongDataPtr  ; now ModuleBuffer
	move.l	ModuleBuffer,a0
...
MTLoop3:
	******BAD BUG******	
	; !!!For once it wasn't my code!!!!
	;;;;;;;;;;;;;;;;clr.l	(a2) 	;Dont need this...Only VERY OLD mods don't have the first 
					;4bytes blank at the start of each sample.
					; (hack used for playing blank sample)
					; *** NOT always 32 samples used in mod, BANG ***
	move.l	a2,(a1)+
	moveq	#0,d1
	move.w	42(a0),d1
	add.l	d1,d1
...
MT_NoNewAllChannels:
	...
	bra	MT_CheckEfx
MT_GetNewNote:
	move.l	ModuleBuffer,a0 ;//MT_SongDataPtr,a0
	lea	12(a0),a3
...
MT_Trenoc:
	move.l	N_Start(a4),(a6)	;Set Start
	
	move.w	N_Length(a4),4(a6)	;Set Length

	move.w	N_Period(a4),d0
	move.w	d0,6(a6)		;Set Period
...
MT_SetDMA:
	...
	move.l	N_LoopStart(a4),$d0+custom
	move.w	N_Replen(a4),$d4+custom

	suba.w	#44,a4

	move.l	N_LoopStart(a4),$c0+custom
	move.w	N_Replen(a4),$c4+custom

	suba.w	#44,a4
	move.l	N_LoopStart(a4),$b0+custom
	move.w	N_Replen(a4),$b4+custom

	suba.w	#44,a4
	move.l	N_LoopStart(a4),$a0+custom
	move.w	N_Replen(a4),$a4+custom
...
MT_JumpLoop:
	...
	beq	MT_Return2
MT_JmpLoop:
	move.b	N_PattPos(a4),MT_PBreakPos
	st	MT_PBreakFlag
	rts
...
MT_RtnSkp:
	...
	bne.s	MT_RtnEnd
MT_DoRetrig:
	move.w	N_DMABit(a4),$096+custom	; Channel DMA off
	move.l	N_Start(a4),(a6)
	
	move.w	N_Length(a4),4(a6)
...
*	bset	#15,d0
	...
	move.l	N_LoopStart(a4),(a6)
	
	move.l	N_Replen(a4),4(a6)
