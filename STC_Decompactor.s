UnPackData:
	...
	move.l	Location_Ptr,a1	;***SOURCE***	 
	cmp.l	#"S404",(a1)
	bne	Not_Crunched

	move.l	8(a1),FileSize	; update filesize 
	
	PRINT_F  	MSG_DECRUNCHING,8(a1)

	;*** CREATE NEW UNCRUNCH SPACE ***
	move.l	a1,-(sp)	;crunched mem location
	ADD_MEM  MemType,8(a1),Location_Ptr
	;*** QUICK DECRUNCH ***
	move.l		Location_Ptr,a0	;***DEST***	
	bsr		Decrunch
	;*** FREE UP THE CRUNCHED DATA ***
	FREEMEMITEM	(sp)+  ;crunched data no longer needed
Not_Crunched:
...
decrunch:
_l0:
		addq	#8,a1			;Skip ID string & security
						;length..
		move.l	a0,a5
		add.l	(a1)+,a0
		add.l	(a1),a1
